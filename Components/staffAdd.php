 <div class="overlayR"></div>

 <div class="residentsForm1">


     <div class="formform1">

         <div class="rtopheadcon">


             <div class="rheadTitle">

                 <div class="rheadcon">
                     <div class="line"></div>
                     <p>ADD STAFF INFORMATION</p>
                     <div class="line"></div>
                     <i class='bx bxs-x-circle' onclick="hideElements()"></i>
                 </div>

             </div>
         </div>
         <!-- form  -->

         <form id="formContainer3" method="post" action="../Php/staffAdd.php">
             <div class="rform1">
                 <div class="rInput">
                     <label for="idnum">ID Number</label>
                     <input type="text" id="idnum" name="idnum" placeholder="Enter ID Number" required>
                 </div>

                 <div class="rInput">
                     <label for="fname">Firstname</label>
                     <input type="text" id="fname" name="fname" placeholder="Enter Firstname" required>
                 </div>

                 <div class="rInput">
                     <label for="lname">Lastname</label>
                     <input type="text" id="lname" name="lname" placeholder="Enter Lastname" required>
                 </div>
             </div>

             <div class="rButcon">
                 <button type="submit" class="rSubmit2">ADD</button>
             </div>
         </form>

     </div>